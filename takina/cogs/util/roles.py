...
class RolesSlash(commands.Cog):
    ...
    @commands.has_permissions(manage_roles=True)
    async def add(self, interaction: nextcord.Interaction, role: nextcord.Role = nextcord.SlashOption(description="The role to add", required=True), member: nextcord.Member = nextcord.SlashOption(description="The member to add the role to", required=True)):
        """Adds a role to a member. Usage: `role add role member`."""
    ...
    @commands.has_permissions(manage_roles=True)
    async def remove(self, interaction: nextcord.Interaction, role: nextcord.Role = nextcord.SlashOption(description="The role to remove", required=True), member: nextcord.Member = nextcord.SlashOption(description="The member to remove the role from", required=True)):
        """Removes a role from a member. Usage: `role remove role member`."""
