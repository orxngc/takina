...
class Roles(commands.Cog):
    ...
    async def role(self, ctx: commands.Context):
        ...
        await ctx.reply(
            "Please specify a subcommand: `add` or `remove`", mention_author=False
        )
    ...
    async def add(self, ctx: commands.Context, role: nextcord.Role, member: str = None):
        ...
        try:
            ...
            await ctx.reply(
                f"Added role `{role.name}` to **{member.display_name}**.",
                mention_author=False,
            )
        except nextcord.Forbidden:
            await ctx.reply(
                "I don't have permission to add that role.", mention_author=False
            )
    ...
    async def remove(
        ...
        try:
            ...
            await ctx.reply(
                "I don't have permission to remove that role.", mention_author=False
            )
        except nextcord.HTTPException as e:
            await ctx.reply(f"An error occurred: {str(e)}", mention_author=False)
    ...
    async def role_error(self, ctx: commands.Context, error):
        ...
        if isinstance(error, commands.MissingPermissions):
            await ctx.reply(
                "You don't have permission to manage roles.", mention_author=False
            )
        elif isinstance(error, commands.BadArgument):
            await ctx.reply(
                "Invalid role or user. Please mention the correct role and member.",
                mention_author=False,
            )
        else:
