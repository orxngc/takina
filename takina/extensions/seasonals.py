...
class AnimeSeasonals(commands.Cog):
    ...
    async def seasonals(self, ctx: commands.Context, season: str = None, year: int = None):
        """Command for displaying seasonal anime from MyAnimeList."""
        ...
        try:
            ...
            for i in range(0, len(seasonals), 5):
                ...
                for anime in seasonals[i:i+5]:
                    ...
                    embed.add_field(name="\u200b", value=f"[{title}]({url})\n {episodes} episodes, rated {trimmed_rating}, ranked {rank} with {members} members and a score of {score}.", inline=False)
    ...
    async def seasonals_slash(self, interaction: Interaction, 
